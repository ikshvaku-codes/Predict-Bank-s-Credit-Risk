...
import sys,  os
from sklearn.metrics import r2_score,mean_squared_error, confusion_matrix, precision_score, recall_score, accuracy_score
import numpy as np
...
def evalute_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    """
    Description:
    ...
    try:
        ...
        for model in model_list:
            ...
            #Calculating Confusion Matrix for training and testing
            ...
            # Calculating recall_score of train_accuracy and test_accuracy
            ...
            logging.info(f"Train Precision Score: [{train_pres}].")
            logging.info(f"Test Precision Score: [{test_pres}].")
            logging.info(f"Train Recall Score: [{train_recall}].")
            logging.info(f"Test Recall Score: [{test_recall}].")
            logging.info(f"Train Confusion Matrix: [{train_con}].")
            logging.info(f"Test Confusion Matrix: [{test_con}].")
            ...
            MODEL_SELECTION_KEY: {
                "module_0": {
                    ...
                    SEARCH_PARAM_GRID_KEY: {
                        ...
                        "n_estimators": [x for x in range(0,510,30)]
                    }

                },
                "module_1": {
                    MODULE_KEY: "sklearn.ensemble",
                    CLASS_KEY: "RandomForestClassifier",
                    PARAM_KEY:
                        {"n_estimators": 100,
                         "criterion": "gini",
                         },
                ...
                "module_3": {
                    MODULE_KEY: "sklearn.naive_bayes",
                    CLASS_KEY: "GaussianNB",
                    PARAM_KEY:
                        {"deep": True,
                         },
                    SEARCH_PARAM_GRID_KEY: {
    ...
    def get_initialized_model_list(self) -> List[InitializedModelDetail]:
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys():
                model_initialized_config = self.models_initialization_config[model_serial_number]
                model_obj_reference = ModelFactory.class_for_name(model_initialized_config[MODULE_KEY],
                                                                  model_initialized_config[CLASS_KEY])
                model = model_obj_reference()
                
                if PARAM_KEY in model_initialized_config:
                    model_obj_property_data = dict(model_initialized_config[PARAM_KEY])
                ...
                model_name = f"{model_initialized_config[MODULE_KEY]}.{model_initialized_config[CLASS_KEY]}"
                
                initialized_model_list.append(
